...
| A2 | "Md files as storage layer" was entirely backend-side; the frontend could not run over files without a server. | Blocker | Filesystem semantics moved into `@hmzisb/notion-docs-core` over a `FileStore` interface. New `filesystem` adapter over `FileSystemDirectoryHandle` (picked folder or OPFS). Playground works with zero backend. | D-03, docs/03 §3-4, docs/08 §7 |
...
| B7 | Conformance suite path drifted between `test/conformance` and `src/testing`. | Low | `src/testing/conformance.ts`, exported from `@hmzisb/notion-docs-core/testing`. | docs/00, docs/02, docs/03 |
