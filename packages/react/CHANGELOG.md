...
- First release.
  ...
  `@docs/core` ships the provider contract and its Zod schemas, the file-store semantics that turn a
  ...
  conformance suite under `@docs/core/testing` is what a new backend is measured against.

  `@docs/react` ships the UI over it: a virtualised page tree with drag, keyboard move and rename, a
