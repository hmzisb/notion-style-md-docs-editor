...
function useTableResizeController({
  ...
  const applyResize = React.useCallback(
    (event: PointerEvent, finished: boolean) => {
      ...
      const nextInitial = effectiveColSizesRef.current[dragState.colIndex + 1] ?? 0;
...
export function TableRowElement({ children, ...props }: PlateElementProps<TTableRowElement>) {
  ...
  const readOnly = useReadOnly();
  const editor = useEditorRef();
  const rowIndex = useElementSelector(([, path]) => path.at(-1)!, [], {
    key: KEYS.tr,
  });
...
function RowDropLine() {
  ...
  return (
    <div
      className={cn(
        'absolute inset-x-0 left-2 z-50 h-0.5 bg-primary',
...
export function TableCellElement({
  ...
  const isSelectingRow = useBlockSelected(rowId) || isSelectingTable; // eslint-disable-line @typescript-eslint/prefer-nullish-coalescing -- both are booleans
  const isSelectionAreaVisible = usePluginOption(BlockSelectionPlugin, 'isSelectionAreaVisible');
  ...
  return (
    <PlateElement
      ...
      className={cn(
        ...
        isHeader && 'bg-muted/50 text-start font-medium *:m-0',
        'before:size-full',
    ...
    >
      <div
        className="relative z-20 box-border h-full px-2 py-1"
